...
export function ProductGrid({ products }: { products: Maillot[] }) {
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {products.map((maillot) => (
        <Link
          ...
          className="relative bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg hover:border-blue-950 transition-all flex flex-col group"
        >
          {maillot.badge && (
            <span
          ...
          <div className="p-3 flex flex-col flex-1">
            <p className="text-[10px] font-semibold text-gray-500 uppercase">{maillot.league}</p>
            <h3 className="text-sm font-bold text-gray-900 line-clamp-2 mt-1 group-hover:text-blue-950">
              {maillot.name}
            </h3>
            <div className="flex items-baseline gap-2 mt-3">
              <span className="text-lg font-black text-gray-900">{maillot.price.toFixed(2)} €</span>
              <span className="text-xs text-gray-400 line-through">{maillot.oldPrice.toFixed(2)} €</span>
            </div>
            <span className="mt-3 block w-full text-center bg-blue-950 text-white text-xs font-bold uppercase py-2.5 rounded group-hover:bg-gray-900 transition-colors">
              Voir le maillot
            </span>
          </div>
        </Link>
